	...
	dbInfluxDB.query(influxQuery).then(data => {
		...
		engine.on("success", function(event, almanac, ruleResult) {
			...
			if( event.type === "mqttPublish" ) {
				let mqttPayload = {date: moment(payload.dtepoch).format("LLL"), dtepoch:payload.dtepoch, value:payload.value, flow: payload.flow};
			...
			t6mqtt.publish(null, mqttInfo+"/ruleEvents/"+user_id, JSON.stringify({"date":moment().format("LLL"), "dtepoch":moment().format("x"),"EventType": event.type, "rule_id": event.params.rule_id, "message":"Rule matching EventType", "environment": process.env.NODE_ENV}), true);
		});
		engine.run(payload);
	}).catch(err => {
		var envelope = {
